...
public class ChatWebsocketHandler extends TextWebSocketHandler{
	...
	// - WebSocketSession을 모아둠
	// == 현재 웹소켓에 연결되어 있는 모든 클라이언트를 알 수 있다.
	// -> Set을 분석해서 원하는 클라이언트를 찾아서 메세지를 전달할수 있다.
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		// WebsocketSession : 웹 소켓에 접속 요청한 클라이언트의 세션
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		if(result > 0 ) {
			...
			// 같은 방에 접속중이 클라이언트에게만 메세지 보내기
			// -> Set<WebSocketSession에서 같은방 클라이언트만 골라내기
			
			for( WebSocketSession s : sessions ) {
				// WebSocketSession == HttpSession(로그인정보, 채팅방 번호)을 가로챈것
				int chatRoomNo = (Integer)s.getAttributes().get("chatRoomNo");
				
				// WebSocketSession에 담겨져 있는 채팅방 번호와
				// 메세지에 담겨있는 채팅방 번호가 같을경우
				// 같은방 클라이언트이다.
				if( chatRoomNo == chatMessage.getChatRoomNo()) {
					
					// 같은방 클랑이언트에게 JackSon이 아닌 JSON 형식 메세지를 보낸다
