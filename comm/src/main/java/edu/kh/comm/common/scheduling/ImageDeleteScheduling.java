...
public class ImageDeleteScheduling {
	...
	// 2) /resouce/images/board 폴더에 존재하는 모든 이미지 파일 목록 조회
	...
	public void serverImageDelete() {
		...
		File[] arr = path.listFiles(); // path가 참조하는 폴더에 있는 모든 파일을 얻어와 FIle배열 반환
		...
		// 3) 두 목록을 비교해서 일치하지 않는 이미지 파일을 삭제
		// (DB에는 없는데 서버 폴더에 있으면 삭제)
		if( !serverList.isEmpty()) { // 서버에 이미지 파일이 있을떄 비교 /삭제 진행
			
			// server : \resorce\image\board\sample2.jpg
			// DB : /resource/image/board/sample2.jpg
			
			
			for(File serverImage :  serverList) {
				
				String name = "/resources/images/board/" + serverImage.getName(); // 파일명ㅇ만 얻어오기
				
				if(dbList.indexOf(name) == -1) { // List에 value와 같은 값이 있으면 인덱스반환/ 없다면 -1 반환
